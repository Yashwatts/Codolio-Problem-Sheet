...
interface EditQuestionModalProps {
  ...
  onSave: (content: string, link: string, difficulty: 'easy' | 'medium' | 'hard') => void
  onCancel: () => void
}
...
export default function EditQuestionModal({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (content.trim()) {
      onSave(content.trim(), link.trim(), difficulty)
    }
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onCancel}>
      <div className="bg-white dark:bg-[#18181b] rounded-lg shadow-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Edit Question</h2>
          <button
            onClick={onCancel}
